...
from .kou_core import KouCore, KouParams


@register_model("kou")
class KouModel(GlobalModel):
    def __init__(self, *, S0: float, r: float, q: float = 0.0, Umax: float = 200.0, n_quad: int = 96):
        self.core = KouCore(S0=S0, r=r, q=q)
    ...
    def call_prices(self, K: np.ndarray, T: float, params: KouParams | dict, **kwargs) -> np.ndarray:
        if isinstance(params, dict):
            params = KouParams.from_dict(params)
        K = np.asarray(K, float).ravel()
        return self.core.call_prices(K, float(T), params, Umax=self.Umax, n_quad=self.n_quad)

    def price_surface(self, K_grid: np.ndarray, T_grid: np.ndarray, params: dict | KouParams, **kwargs) -> np.ndarray:
        if isinstance(params, KouParams):
            p = params
        else:
            p = KouParams.from_dict(params)
