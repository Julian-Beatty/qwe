...
def rnd_surface_plot(
    ...
    *,
    ...
    fig.colorbar(surf, ax=ax, shrink=0.6, aspect=15, label="q(K|T)")
    fig.tight_layout()
...
def iv_surface_plot(
    ...
    *,
    ...
    Saving:
      - static: any image extension
      - interactive: .html (recommended), images need kaleido
    """
    if "iv_surface" not in res:
...
def call_surface_vs_observed(
    ...
    *,
    ...
    """
    3D plot: fitted call surface (K,T -> C) with observed quotes overlay.

    Requires in res:
      - res["C_fit"] shape (len(T_grid), len(K_grid))
      - res["K_grid"], res["T_grid"]
    Requires in day:
      - day.K_obs, day.T_obs, day.C_obs

    Notes:
      - spot line is optional (drawn at K=spot on the base plane).
      - save: if provided, creates parent directories and writes PNG.
    """
    # ---- fitted surface
    if "C_fit" not in res:
    ...
    # fitted surface
    ax.plot_surface(KKp, TTp, Cp, linewidth=0, antialiased=True, alpha=float(surface_alpha))

    # observed scatter
    ax.scatter(K_obs, T_obs, C_obs, s=float(obs_size), alpha=float(obs_alpha), label="Observed")

    # optional spot "curtain" on base plane (K=spot, varying T, C=0)
    if spot is not None and np.isfinite(spot):
        tmin, tmax = float(np.min(T_grid)), float(np.max(T_grid))
        ax.plot([spot, spot], [tmin, tmax], [0.0, 0.0], linestyle="--", linewidth=2.0, label="Spot")
    ...
    # legend (only shows scatter + spot; surface itself doesn’t have a legend entry)
    ...
    if save is not None:
        save = Path(save)
        # if user passed ".../BTC_calls2020-04-16" without extension, add .png
        if save.suffix == "":
